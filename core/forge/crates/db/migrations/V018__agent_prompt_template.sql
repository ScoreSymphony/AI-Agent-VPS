...
CREATE TABLE agent_new (
    id TEXT PRIMARY KEY,
    name TEXT NOT NULL,
    executor_type TEXT NOT NULL CHECK (executor_type IN ('shell', 'codex', 'claude_code', 'opencode', 'null')),
    model TEXT,
...
INSERT INTO agent_new (
    id, name, executor_type, model, reasoning_effort, permission_policy,
    capabilities_json, config_json, daemon_id, max_concurrent_tasks,
    heartbeat_interval_seconds, max_missed_heartbeats, status,
    last_heartbeat_at, is_default, version, created_at, updated_at
)
SELECT
    id, name, executor_type, model, reasoning_effort, permission_policy,
    capabilities_json, config_json, daemon_id, max_concurrent_tasks,
