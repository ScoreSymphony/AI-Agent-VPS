...
CREATE TABLE task_new (
    ...
    title               TEXT NOT NULL,
    description         TEXT,
    task_type           TEXT NOT NULL DEFAULT 'task' CHECK (task_type IN ('task', 'planning_task', 'sub_task')),
...
INSERT INTO task_new (
    ...
    title, description, task_type, status, priority, merge_config, metadata_json,
...
SELECT
    ...
    assignee_type, assignee_id,
    title, description,
    CASE WHEN parent_task_id IS NOT NULL THEN 'sub_task' ELSE 'task' END,
    status, priority, merge_config, metadata_json,
    plan, task_state_config, error_annotation, review_passed_at,
    deleted_at, version, created_at, updated_at, subtask_order, board_position,
    blocked_json, failed_json, entry_barrier_json, archived_at
FROM task;
