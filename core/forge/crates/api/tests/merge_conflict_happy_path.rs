...
use api_types::{
    ...
    ProjectResponse, RepoResponse, TaskResponse, TaskStatus,
};
use axum::{
    body::{to_bytes, Body},
...
async fn merge_conflict_follow_up_resolves_and_reaches_done() {
    ...
    let created_task: TaskResponse = json_request(
        ...
        json!({ "title": "Merge conflict happy path task",
            "description": "complete successfully, pass auditor review, then resolve merge conflict",
    ...
    assert!(
        ...
        "follow-up has parent linkage"
    );

    let completed = poll_until_task_status(&harness.app, &created_task.id, "done".to_owned()).await;
    assert_eq!(completed.status, "done".to_owned());
    assert!(
        completed.review_passed_at.is_some(),
        "review_passed_at remains set at done"
    );
    assert!(
        !workspaces_root.path().join(&created_task.id).exists(),
        "workspace task directory is cleaned after merge-conflict recovery reaches done"
    );
    ...
    assert_eq!(
        transition_logs
            ...
            .count(),
        1,
        "exactly one merge_failed transition should be logged"
    );
    assert_eq!(
        transition_logs
            .iter()
            .filter(|entry| entry.to_state == "done")
            .count(),
        1,
        "exactly one done transition should be logged"
    );
}
...
async fn register_daemon_and_report_codex(app: &Router, workspace_root: &Path) -> String {
    let registration: DaemonRegisterResponse = json_request(
        ...
        json!({
            ...
            "hostname": "merge-conflict-happy-host",
            "os": "linux",
            "arch": "x86_64",
            "agent_version": "merge-conflict-happy-test",
            "labels": { "suite": "merge_conflict_happy_path" }
        }),
        StatusCode::OK,
    )
    .await;
...
async fn poll_until_task_status(
    ...
    panic!("task did not reach {expected_status:?} within timeout; last task: {last_task:?}");
}
