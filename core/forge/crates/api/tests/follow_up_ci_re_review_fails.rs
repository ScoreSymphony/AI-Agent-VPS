...
async fn ci_failure_on_re_review_clears_review_passed_at_and_blocks_task() {
    ...
    let created_task: TaskResponse = json_request(
        ...
        json!({ "title": "CI re-review task",
            "description": "make a conflicting implementation",
    ...
    .await;
    set_auditor_review_config(&harness, &created_task.id, &auditor_agent.id, "true").await;
    ...
    allow_follow_up.store(true, Ordering::SeqCst);
...
struct CiReReviewCodexAdapter {
    repo_path: PathBuf,
    executor_calls: Arc<AtomicUsize>,
    allow_follow_up: Arc<AtomicBool>,
}

impl CiReReviewCodexAdapter {
    fn new(repo_path: PathBuf) -> Self {
        Self {
            repo_path,
            executor_calls: Arc::new(AtomicUsize::new(0)),
            allow_follow_up: Arc::new(AtomicBool::new(false)),
        }
    }

    fn allow_follow_up(&self) -> Arc<AtomicBool> {
        Arc::clone(&self.allow_follow_up)
    }
}

impl CodingExecutorAdapter for CiReReviewCodexAdapter {
...
async fn register_daemon_and_report_codex(app: &Router, workspace_root: &Path) -> String {
    let registration: DaemonRegisterResponse = json_request(
        ...
        json!({
            ...
            "hostname": "ci-re-review-host",
            "os": "linux",
            "arch": "x86_64",
            "agent_version": "ci-re-review-test",
            "labels": { "suite": "follow_up_ci_re_review_fails" }
        }),
        StatusCode::OK,
    )
    .await;
...
async fn set_auditor_review_config(
    harness: &TestHarness,
    task_id: &str,
...
async fn poll_until_merge_fix_dispatched(app: &Router, task_id: &str) -> TaskResponse {
    ...
    for _ in 0..200 {
        ...
        .await;
        let has_running_merge_fix =
            executions_for_task(app, task_id)
                .await
                .iter()
                .any(|execution| {
    ...
    panic!("task did not dispatch merge-fix follow-up within timeout; last task: {last_task:?}");
}
