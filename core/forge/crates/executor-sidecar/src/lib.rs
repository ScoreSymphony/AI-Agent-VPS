...
impl SidecarState {
    pub async fn new(config: SidecarConfig) -> io::Result<Self> {
        ...
        Ok(Self {
            ...
            jobs: Arc::new(RwLock::new(JobStore::default())),
            workspace_root,
            logs_root,
...
impl JobRecord {
    fn snapshot(&self, stdout: String, stderr: String, logs_has_more: bool) -> ExecutionSnapshot {
        ...
        if let Some(object) = metadata.as_object_mut() {
            object.insert("logs_has_more".to_owned(), Value::Bool(logs_has_more));
        }
        ExecutionSnapshot {
...
async fn submit_execution(
    ...
    if let Some(existing) = lookup_by_request(&state, &request.request_ref).await {
        let snapshot = snapshot_with_logs(&existing).await?;
...

    {
        ...
        if let Some(existing_id) = jobs.by_request.get(&request.request_ref).cloned() {
            if let Some(existing) = jobs.by_execution.get(&existing_id).cloned() {
                drop(jobs);
...
fn apply_result(record: &mut JobRecord, result: &ExecutionResult) {
    ...
    record.result_code = match record.status {
        ...
        SidecarExecutionStatus::Cancelled | SidecarExecutionStatus::TimedOut => None,
...
mod tests {
    ...
    async fn submit_is_idempotent_and_null_execution_completes() {
        ...
        assert_eq!(get_json["output"]["summary"], "Null executor completed successfully.");
    }
    ...
    #[tokio::test]
    async fn cancellation_is_forwarded_to_fallback_executor() {
        let (_dir, state, workspace) = test_state().await;
        let app = build_router(state);
        ...
        let execution_id = submit_json["execution_id"].as_str().unwrap();
